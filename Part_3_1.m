...
delta = 1;
parameter.tolerance = delta;
parameter.synHop = 500;
y_wsola3 = wsolaTSM(x,alpha,parameter);

subplot(3,2,5)
plot(y_wsola3);
title('for delta = 1 and syncHop = 500');
xlabel('Time (s)');
ylabel('Amplitude');
...
%write the data
audiowrite("alpha 0.5 OLA.wav", y_ola, fs);
audiowrite("alpha 0.5 SWOLA regular.wav", y_wsola, fs);
audiowrite("alpha 0.5 SWOLA stattur.wav", y_wsola2, fs);
audiowrite("alpha 0.5 SWOLA skiping.wav", y_wsola3, fs);
